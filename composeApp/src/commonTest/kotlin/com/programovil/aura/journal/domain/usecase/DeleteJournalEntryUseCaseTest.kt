...
class DeleteJournalEntryUseCaseTest {
    ...
    private val useCase = DeleteJournalEntryUseCase(repository)

    @Test
    fun `invoke forwards entry to repository`() = runTest {
        val entry = JournalEntry("j-1", "Title", "Body", 100L, 200L)
        coEvery { repository.deleteEntry(entry) } returns Result.success(Unit)

        val result = useCase(entry)

        assertTrue(result.isSuccess)
        coVerify { repository.deleteEntry(entry) }.wasInvoked(exactly = 1)
    }
    ...
    fun `invoke propagates failure from repository`() = runTest {
        ...
        coEvery { repository.deleteEntry(entry) } returns Result.failure(error)
